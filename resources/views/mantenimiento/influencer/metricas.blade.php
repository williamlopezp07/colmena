...
<div class="tab-content">
  ...
  <div class="tab-pane container <?php echo ($item == 1) ? 'active' : $item ; ?>" id="{{$key->red_social}}"><br>
        ...
        <?php if ($key1->valor=='general'): ?>
          <div class="col-md-3">
            ...
            <?php if ($key1->type_input=='text' or $key1->type_input=='number'): ?>
              <input type="{{$key1->type_input}}" class="form-control form-control-sm" name="{{$key1->input}}" <?php $key1->type_input=='number'? ' step="any"' :'' ?> value="">
            <?php elseif($key1->type_input=='promedio'): ?>
              <input type="number" class="form-control form-control-sm" name="{{$key1->input}}"  min="1" max="100" step="any" value="" readonly>
            <?php elseif($key1->type_input=='moneda'): ?>
              <select class="form-control form-control-sm" name="{{$key1->input}}">
                <option value="USD">USD</option>
                <option value="PEN">PEN</option>
              </select>
            <?php elseif($key1->type_input=='textarea'): ?>
              <textarea class="form-control form-control-sm" name="{{$key1->input}}" rows="3" ></textarea>
            <?php elseif($key1->type_input=='array'): ?>
              <input type="text" class="form-control form-control-sm array_prom" name="{{$key1->input}}" value="">
            <?php elseif($key1->type_input=='er'): ?>
              <input type="number" class="form-control form-control-sm er" name="{{$key1->input}}" value=""  readonly>
            <?php elseif($key1->type_input=='castigo'): ?>
              <input type="number" class="form-control form-control-sm castigo" name="{{$key1->input}}" step="any" min="0" max="100" value="100" >
          <?php endif; ?>
          </div>
        <?php endif; ?>
      <?php endforeach; ?>
        ...
        <?php if ($key1->valor=='metrica' or $key1->valor=='metricas'): ?>
          <div class="col-md-3">
            ...
            <?php elseif($key1->type_input=='er'): ?>
              <input type="number" class="form-control form-control-sm er" name="{{$key1->input}}" value="" readonly>
            <?php elseif($key1->type_input=='castigo'): ?>
              <input type="number" class="form-control form-control-sm castigo" name="{{$key1->input}}" min="0" max="100" step="any" value="100" >
          <?php endif; ?>
          </div>
        <?php endif; ?>
      <?php endforeach; ?>
    ...
    <div class="row">
      <center><h1>DISTRIBUCIÓN POR EDAD</h1></center>
      <?php foreach (json_decode($key->json_demografica) as $key1): ?>
        <?php if (strpos($key1->input, "general") !== false): ?>
          <?php if (strpos($key1->input, "sexo_") !== false) {
            $col = 12;
          }elseif (strpos($key1->input, "edad_") !== false) {
            $col = 4;
          ...
          <?php if (strpos($key1->input, "edad_") !== false): ?>
            <div class="col-md-<?php echo $col ?>">
              <label for="" style="font-size:9pt">{{$key1->descripcion}} </label>
              <?php if ($key1->type_input=='text' or $key1->type_input=='number'): ?>
                <input type="{{$key1->type_input}}" class="form-control form-control-sm" name="{{$key1->input}}" <?php $key1->type_input=='number'? ' step="any"' :'step="any"' ?> value="">
              <?php endif; ?>
            </div>
          <?php endif; ?>
        <?php endif; ?>
    ...
    <hr>
    <div class="row">
      <center><h1>DISTRIBUCIÓN POR LOCACIÓN</h1></center>
...
$('input[name=handle]').val('{{$influencer_cod}}')
</script>
@else
<ul class="nav nav-tabs">
  <?php $item = 1; ?>
...
<div class="tab-content">
  ...
  <div class="tab-pane container <?php echo ($item == 1) ? 'active' : $item ; ?>" id="{{$key->red_social}}"><br>
    <div class="row">
      <div class="col-md-12">
        <a href="{{'influencer/exportMetrica/'.$influencer_cod.'/'.$key->red_social}}" target="_blank" class="btn btn-success">Exportar a Excel</a>
      </div>
    </div>
    <div class="row">
      <center><h1>GENERAL</h1></center>
        ...
        <?php if ($key1->valor=='tarifario'): ?>
          <div class="col-md-6">
            ...
            <?php if ($key1->type_input=='text' or $key1->type_input=='number'): ?>
              <input type="{{$key1->type_input}}" class="form-control form-control-sm" name="{{$key1->input}}" <?php $key1->type_input=='number'? ' step="any"' :'' ?> value="">
            <?php elseif($key1->type_input=='promedio'): ?>
              <input type="number" class="form-control form-control-sm" name="{{$key1->input}}"  min="1" max="100" value="" step="any" readonly>
            <?php elseif($key1->type_input=='moneda'): ?>
              <select class="form-control form-control-sm" name="{{$key1->input}}">
                <option value="USD">USD</option>
                <option value="PEN">PEN</option>
...
<script type="text/javascript">
  ...
  metricasEdit.forEach(element => (element[0].metricas).forEach( val =>
    // $('#'+element[0].red_social).find('[name='+val.input+']').val(val.valor)
         val.input=='handle' ?
          $('#'+element[0].red_social).find('[name='+val.input+']').val(valor = val.valor=='' ? '{{$influencer_cod}}' : val.valor)
          :
          $('#'+element[0].red_social).find('[name='+val.input+']').val(val.valor)
  )
);
